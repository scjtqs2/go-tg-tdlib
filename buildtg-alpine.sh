...
docker buildx build --tag scjtqs/go-tg:${BUILD_VERSION} --platform linux/amd64,linux/arm64,linux/386,linux/arm/v7 --build-arg BUILD_VERSION="${BUILD_VERSION}" --build-arg TD_GIT_COMMIT="${TD_GIT_COMMIT}" --push -f alpine.go-td.Dockerfile .
